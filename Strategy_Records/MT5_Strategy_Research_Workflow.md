...
### Step 0.5：成本可行性預檢（**寫 EA 之前必做**，2026-07-20 新增）

零售交易成本是一道接近固定的**地板**：每筆 round-turn 約 `0.9–1.3 pips`（spread + commission + slippage）。策略能否存活，主要取決於**每筆預期 edge 的絕對大小**，而不是勝率或 PF。

因此在寫任何程式之前，先估算：

```text
每筆預期 edge（以 pips 或報價單位表示）  vs  成本地板
```

| 比值 `edge / cost` | 判定 |
|---|---|
| `< 1.5` | **不實作**。即使回測為正，扣成本後必為負或雜訊 |
| `1.5 – 3` | 邊際。只有在其他面向（正交性、樣本數）特別有價值時才做，且須事前寫下成本後門檻 |
| `> 3` | 成本不是主要風險，可進入實作 |

**關鍵性質：commission 換算成 pips 之後與部位大小無關**（commission 與 pip value 都隨手數等比放大）。因此「edge 太薄」**不能靠調整部位大小或風險百分比解決**，這是結構性的。

#### 實證依據（2026-07-20，兩條線同日結案）

| 策略 | 頻率 | 每筆 edge | edge/cost | 結果 |
|---|---:|---|---:|---|
| `Session_Range` USDJPY | ~90 筆/年 | ~1.0 pip | ~1.1 | 成本後 PF 0.98–1.02 → 結案 |
| `Time_Window` Gold | ~250 筆/年 | 0.110 USD/oz | ~0.4 | 成本後 PF 0.956 → 結案 |
| **`SR_Channel_Breakout` EURUSD** | **~16 筆/年** | **RR 2.0，數十 pips** | **~25** | **成本後 PF 1.42 → 晉級** |

兩條結案的線在成本關卡之前都有「看起來還行」的指標——`Session_Range` 的 OOS PF 甚至達 `1.235`，且 OOS 優於 Development、無樣本外衰減。**訊號是真的，只是比成本小。** 這道預檢若提前執行，可在數分鐘內取得與整個開發＋多次回測週期相同的結論。

#### 推論：低頻線不要加進場過濾器

若策略本身交易數已少（例如 `SR_Channel_Breakout` EURUSD 六年僅 103 筆），**任何進場過濾器都是錯誤方向**——它必然減少樣本，而樣本數正是該線的瓶頸。`S9` 的 volume filter 即為實例：IS 的 PF 由 1.47 提升到 1.709，但交易數降至 60、forward 期不足 30 筆而失去統計效力。

對這類策略，應優先考慮**不減少交易數、但放大每筆 edge 的改動**，主要是出場（trailing 幾何、TP 結構、部分平倉）。
