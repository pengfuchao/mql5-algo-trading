...
### 7.0 Step 0.5 成本可行性預檢 — **已補跑，通過**（2026-07-22）

本日誌成文於 2026-07-02，早於 [Step 0.5](MT5_Strategy_Research_Workflow.md) 於 07-20 導入，故該關從未執行。以 USDJPY M15 最佳候選（216 筆、`Expected Payoff = 0.283 USD/trade`、fixed 0.01 lot）補算：

**回測 commission 設為 0**，點差已含（real ticks 使用歷史點差），故 0.283 為未扣手續費之值。0.01 手 USDJPY 之 1 pip ≈ 0.067 USD：

| | USD/trade | ≈ pips |
|---|---:|---:|
| 回測 expected payoff（已扣點差）| 0.283 | 4.2 |
| 待扣 commission（$7/手來回 × 0.01 手）| −0.070 | −1.05 |
| **扣後淨 edge** | **0.213** | **3.2** |

commission 約吃掉 25% 的 edge；全樣本淨利由 61.19 降至約 46 USD，PF 由 1.391 降至約 1.28，**仍高於 §8 的 PF ≥ 1.15 門檻**。

**結論：成本不是本線的主要風險**，與 `Session_Range`（edge/cost ≈ 1.1）、`Time_Window`（≈ 0.4）兩條結案線的失效模式不同。

### 7.0.1 「兩三年打平」是否代表 regime 依賴 — **目前資料無法分辨**

216 筆 / 6 年 ≈ **36 筆/年**。由 PF 1.391 與淨利 61.19 反推：毛利 ≈ 218、毛損 ≈ 157，總周轉 ≈ 375 USD / 216 筆 → 平均每筆絕對值 ≈ 1.7 USD，per-trade 標準差粗估 ≈ 2 USD。

- 單一年度期望值：`36 × 0.283 ≈ +10.2 USD`
- 單一年度標準差：`2 × √36 ≈ ±12 USD`

**期望值小於標準差。** 在此訊噪比下，單年為負的機率約 20%，六年出現 1–2 個負年加上一兩個接近打平的年份屬正常抽樣結果。

**即使 edge 真實且完全不依賴 regime，年度分布也會與目前觀察到的幾乎相同。** 因此不可由「年度打平」推論 regime 依賴——該推論是把雜訊讀成訊號。（per-trade 標準差為反推估計，非實測；但即使偏差 50%，結論方向不變。）

**本線的處境與已結案的 SR Channel 不同**：SR Channel 是參數空間的刀鋒（已被證偽）；本線是**樣本不足以判斷**（既未證實亦未證偽）。對應的解法是增加樣本，見 7.5。

### 7.5 多商品樣本擴充（**排程中，尚未執行**）

目的：以增加樣本區分「真實但薄的 edge」與「僅特定條件有效」。**門檻須於執行前寫定，跑完不得調整。**

固定設定（全程不動）：USDJPY 最佳候選之完整設定 —— `M15 / HTF=H1 / MaxSpread=15 / MaxHoldingBars=16 / Session=12–16`，fixed 0.01 lot，期間與現有長樣本一致。

**事前指定商品清單**（涵蓋不同流動性與相關性，執行前不得增刪）：

| # | 商品 | 選入理由 |
|---|---|---|
| 1 | USDJPY | 現有最佳候選（基準）|
| 2 | EURUSD | 已測，紀錄為「次要候選、edge 較薄」，作為弱獨立佐證 |
| 3 | GBPUSD | 與 EURUSD 高相關，檢驗是否隨之成立 |
| 4 | AUDUSD | 商品貨幣，與 1–3 相關性較低 |
| 5 | USDCHF | 與 EURUSD 高負相關，檢驗方向性偏誤 |
| 6 | EURJPY | 交叉盤，檢驗是否為 JPY 特性而非 USDJPY 特性 |

**判定門檻（事前寫定）**：

- 每個商品皆以**扣除 commission 後**的數字評估（`Adjusted = Net Profit − Trades × 0.07`）。
- 通過：**6 個商品中 ≥ 4 個**扣成本後 `PF ≥ 1.15` 且 `Expected Payoff > 0`。
- 若僅 USDJPY 單獨成立、其餘全面打平或為負 → 判定為單一商品特性，**不具泛化性，結案**。
- ⚠️ **不得於跑完後挑選表現好的商品組成子集**——該行為即為選櫻桃，R4 的教訓（見 [Strategy_SR_Channel_Breakout](Strategy_SR_Channel_Breakout.md) §6.6）。

**注意**：本項為樣本擴充，非參數最佳化。全程不得調整任何 input 以迎合個別商品。

**排序**：排在既有第 1 項（demo forward）之後或並行；優先於 7.1 以下各項。
