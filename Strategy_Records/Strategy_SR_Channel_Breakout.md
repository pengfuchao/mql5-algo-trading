...
狀態：**全線結案（2026-07-22）**。S1–S7 因 `iCustom` 參數錯位而全部作廢（§S10）；還原出真實參數組後重驗，R2 樣本外與 R3 逐年皆通過，但 **R4 參數敏感度判定為刀鋒而非高原**（§6.6）。部署卡與 preset 已移出 `Strategy_Live_Candidates/`。**本策略不可部署。**
...
- ~~**結論：EURUSD 專用 breakout 通過全部回測關卡 → 晉級 demo forward。**~~ **已作廢（§S10）**：本節所有數字產生於錯位參數，且此處的「參數 walk-forward」掃描的是指標從未收到的維度。
- 部署卡與 preset 已於 2026-07-22 隨結案移出 `Strategy_Live_Candidates/`（內容見 git 歷史）。
...
## 6.6 重驗結果與結案（2026-07-22）

參數組全程 `PivotPeriod=4 / Source=Close-Open / CWP=1 / MinStr=3 / MaxSR=10 / Loopback=100`，每份報告皆已核對 input 無錯位。

### R2 樣本外分割 — 通過

| | 期間 | 交易 | PF | 淨利 |
|---|---|---:|---:|---:|
| Development | 2020.06–2024.06 | 59 | 1.304 | +906.79 |
| **OOS** | 2024.06–2026.06 | **44** | **1.654** | **+1517.89** |

門檻 OOS `PF ≥ 1.20` 且交易數 `≥ 30`：**兩項皆過**，且 OOS 優於 Development，無樣本外衰減。59+44=103 與全期一致。
...
|---|---:|---:|---:|
| 2020–21 | 1 | — | +195.52 |
| 2021–22 | 13 | 1.104 | +73.24 |
| 2022–23 | 17 | 1.278 | +259.64 |
| 2023–24 | 28 | 1.293 | +364.39 |
| 2024–25 | 29 | 1.672 | +1009.10 |
| 2025–26 | 15 | 1.615 | +457.86 |

**6/6 年為正**；扣除最佳年後其餘合計 +1350.65 仍為正。**通過**。

觀察：2020–21 僅 1 筆，實質為五年樣本；獲利偏向近期（2024–2026 佔 62%），2021–22 的 PF 1.104 幾近打平。

### R4 參數敏感度 — **未通過（結案關卡）**

> ⚠️ `PivotPeriod=3` 為**無效測試點**：指標夾限 `g_prd = MathMax(4, MathMin(30, PivotPeriod))` 將 3 夾回 4，該次結果（103 筆 / PF 1.466 / +2572.71）與原點完全相同，實為原點重跑。選點時未考慮夾限，記為本次流程疏失。

| 鄰點 | 交易 | PF | 淨利 | 判定 |
|---|---:|---:|---:|---|
| `PivotPeriod=5` | 32 | 1.637 | +1016.30 | 過（樣本僅 32）|
| `ChannelWidthPct=2` | 474 | 0.842 | −3036.92 | 崩壞 |
| `MinStrength=2` | 1208 | 0.867 | −5144.75 | 崩壞 |
| `MinStrength=4` | 2 | 2.540 | +115.61 | 無樣本，不計 |
| `Loopback=150` | 268 | 1.081 | +1286.48 | 低於門檻 |

有統計意義的鄰點中僅 `1/4` 維持 `PF ≥ 1.20`（寬鬆計入 `MinStrength=4` 亦僅 `2/5 = 40%`），低於事前門檻 `50%`。
...
**交易數越少 PF 越高，無一例外。** 若存在真實 edge，增加樣本應使 PF 收斂至某個正值，而非穿過 1.0 持續下滑。此處呈現的是：**樣本一旦大到足以測量，策略即虧損**；`PF > 1.2` 的三個配置樣本分別為 103、32、2 筆。原點的 PF 1.466 屬於未收斂的小樣本，不是精選機會下的獲利。

`MinStrength` 單軸更直接顯示刀鋒性質：`2 → 1208 筆`、`3 → 103 筆`、`4 → 2 筆`，單一整數步進即在「過度交易」與「幾乎不交易」之間跳躍。

### 結案判定

依 §6.5 事前寫定的規則（「若鄰點全面崩壞，即使 R2/R3 通過仍結案」）：**本線結案，不執行 R6/R5。**

- 該參數點**不可部署**：證據不成立，且為刀鋒——broker 資料差異造成的 pivot 偵測微小偏移即可能落入相鄰行為區。
- **未完整證偽 SR channel 突破概念本身**（僅測一個落點與五個鄰點），但上表對概念亦不樂觀：有樣本的區域全數虧損。
- 結構性障礙：EURUSD H1 約 17 筆/年，**樣本不足以驗證任何參數選擇**；而唯一能提高交易數的方向（放寬 `MinStrength` / 加大 `ChannelWidthPct`）PF 均 < 0.9。「可測量」與「有獲利」無法同時成立。若日後重啟此概念，須先以多商品／多週期解決樣本問題，而非在 EURUSD H1 上繼續搜參數。

### 方法論記錄

R2（OOS PF 1.654）與 R3（6/6 年正）的表現足以讓人直接部署，而策略實為刀鋒上的雜訊。**原因是 R2/R3 變動的是時間，R4 變動的是參數**；刀鋒點在整段歷史上都是同一個刀鋒，時間類檢驗無法察覺。

**教訓：樣本外與逐年檢驗不能替代參數敏感度檢驗。** 三者測的是不同的失效模式，缺一不可。若本次未事前寫定 R4 與其門檻，本線會帶著「通過樣本外驗證」的標籤進入 demo。
