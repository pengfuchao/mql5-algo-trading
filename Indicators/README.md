...
*   **`Support_Resistance_Channels.mq5`**
    ...
    *   **參數**: `PivotPeriod` (4–30)、`Source` (High/Low 或 Close/Open)、`ChannelWidthPct` (1–8)、`MinStrength`、`MaxNumSR` (1–10，沿用原版顯示語意)、`Loopback` (100–400)、`ChannelWidthMode` (`WIDTH_RANGE_PCT` 或 `WIDTH_ATR`)、`ATRLen`、`ATRMult`、`UseVolumeFilter`、`VolMaLen`、`VolMult`、三色設定、`ShowPivot`、`ShowBroken`、`ShowBounce`、`AlertsOn`，以及兩條可選 MA (SMA/EMA)。
    ...
    *   **EA Buffer Contract** (供 `iCustom` 讀取，建議 `shift = 1` 已收盤棒)：
        ...
        *   `iCustom` 計算參數順序：`PivotPeriod, Source, ChannelWidthPct, MinStrength, MaxNumSR, Loopback, ChannelWidthMode, ATRLen, ATRMult, UseVolumeFilter, VolMaLen, VolMult`（其餘顏色/Extras/MA 參數可省略用預設）。
    *   **實作說明 / 限制**:
        ...
        *   Phase 1 ATR / volume 升級不新增 buffer：ATR 模式只改變通道幾何；volume filter 只 gate `Resistance Broken` / `Support Broken`，不影響 bounce/rejection buffer。
        *   突破與反彈標記在最新收盤棒判定後以物件累積繪製，**不回填全部歷史**。Pine 排序段因 selection 已為降冪實際為 no-op，本版採乾淨交換取代原版 partial-swap quirk，顯示的通道集合不受影響。
        *   新增 ATR channel width mode 與 volume-confirmed breakout 後需重新以 MetaEditor 編譯確認，並以預設參數做 channel / breakout 回歸檢查。
