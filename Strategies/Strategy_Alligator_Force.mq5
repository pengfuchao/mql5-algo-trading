...
int OnInit()
  {
   ...
   // 參數: jaw_period, jaw_shift, teeth_period, teeth_shift, lips_period, lips_shift, ma_method, applied_price
   // 原始 MT4: iAlligator("EURUSD",30, 8,0, 5,0, 3,0, MODE_EMA, PRICE_WEIGHTED, ...)
   ...
   if (myPositions == 0)
     {
      ...
      if(tradeLots > 0)
        {
         if (signal == "Buy")  trade.Buy(tradeLots, Symbol(), SymbolInfoDouble(Symbol(), SYMBOL_ASK), sl_buy, tp_buy, "Alligator Force Buy");
         if (signal == "Sell") trade.Sell(tradeLots, Symbol(), SymbolInfoDouble(Symbol(), SYMBOL_BID), sl_sell, tp_sell, "Alligator Force Sell");
        }
     }
      ...
      if(ticket > 0 && HistoryDealGetInteger(ticket, DEAL_ENTRY) == DEAL_ENTRY_OUT)
        {
         if(HistoryDealGetDouble(ticket, DEAL_PROFIT) < 0)
           {
            iLots = 0.01; // 上一個訂單虧損，本次開倉量減至0.01
           }
         break;
        }
     }
...
string ReturnMarketInfomation()
  {
   ...
   if(CopyBuffer(alligatorHandle, 0, 0, 2, jaw) < 2) return MktInfo;   // GATORJAW
   if(CopyBuffer(alligatorHandle, 1, 0, 2, teeth) < 2) return MktInfo; // GATORTEETH
   if(CopyBuffer(alligatorHandle, 2, 0, 2, lips) < 2) return MktInfo;  // GATORLIPS
