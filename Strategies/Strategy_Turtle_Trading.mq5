...
input bool         UseTrendFilter = true;       // 啟用趨勢過濾 (只順勢突破)
input int          TrendMAPeriod  = 100;        // 趨勢均線週期
input ENUM_MA_METHOD TrendMAMethod = MODE_EMA;  // 趨勢均線類型
...
int OnInit()
  {
   ...
   if(UseTrendFilter)
     {
      maHandle = iMA(Symbol(), PERIOD_CURRENT, TrendMAPeriod, 0, TrendMAMethod, PRICE_CLOSE);
...
void OnTick()
  {
   ...
   // 趨勢過濾：只在價格站上/跌破趨勢均線時，允許對應方向的突破掛單
   bool allowLong = true, allowShort = true;
   if(UseTrendFilter)
     {
      double maArr[1], closeArr[1];
      if(CopyBuffer(maHandle, 0, 1, 1, maArr) < 1) return;
      if(CopyClose(Symbol(), PERIOD_CURRENT, 1, 1, closeArr) < 1) return;
      allowLong  = (closeArr[0] > maArr[0]);   // 上升趨勢才做多突破
      allowShort = (closeArr[0] < maArr[0]);   // 下降趨勢才做空突破
     }

   // (A) 如果完全空倉，放置初始突破掛單 (僅順勢方向)
   if(totalPositions == 0)
     {
      ...
      // 趨勢轉向時，撤掉不再符合方向的既有突破掛單
      if(!allowLong  && buyStopTicket  > 0) { trade.OrderDelete(buyStopTicket);  buyStopTicket  = 0; }
      if(!allowShort && sellStopTicket > 0) { trade.OrderDelete(sellStopTicket); sellStopTicket = 0; }

      if(allowLong && (buyStopTicket == 0 || EMAX != LastEMAX_Base))
        {
      ...
      if(allowShort && (sellStopTicket == 0 || EMIN != LastEMIN_Base))
        {
