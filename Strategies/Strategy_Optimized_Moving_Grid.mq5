...
input int InpPendingNum = 5;
input double InpGridDensity = 2.0;
input int GridSpace = 0;

int PendingNum;
double GridDensity;
input double TakeProfitFactor = 1.5;
input double StopLossFactor = 1.0;
input int EMAPeriod = 120;
...
int OnInit()
  {
   ...
   emaHandle = iMA(Symbol(), PERIOD_H1, EMAPeriod, 0, MODE_SMA, PRICE_CLOSE);
...
int iTradingSignals()
  {
   ...
   if(CopyBuffer(emaHandle, 0, 1, 1, myEMA) > 0)
     {
      if(SymbolInfoDouble(Symbol(), SYMBOL_ASK) < myEMA[0]) return 0; // Note: original code has ask < myEMA return 0 for optimized
      ...
      if(ticket > 0 && PositionGetString(POSITION_SYMBOL) == Symbol() && PositionGetInteger(POSITION_MAGIC) == MyMagicNum && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
        {
         ...
         if(bid > (openPrice + currentGridSpace * _Point * TakeProfitFactor))
           {
         ...
         if(StringFind(comment, MyOrderComment) == 0)
           {
            ...
            if(parsePrice > 0) myTPPrice = parsePrice + currentGridSpace * _Point * TakeProfitFactor;
            else myTPPrice = openPrice + currentGridSpace * _Point * TakeProfitFactor;
           }
         else
           {
            myTPPrice = openPrice + currentGridSpace * _Point * TakeProfitFactor;
           }
