...
// ⚠️ 未驗證機制：原為 2026-07-20 規劃的出場 A/B 而寫，但對照組回測意外揭露
//    `iCustom` 參數錯位，本策略線隨後結案（見 Strategy_Records §S10 / §6.6），
//    **該 A/B 從未實際執行**。程式碼保留供未來低頻線復用，預設關閉。
//    啟用前須自行完成 A/B 並事前寫定判定門檻。
input group "Exit — Break-even (未驗證，預設關閉)"
input bool            InpUseBreakEven      = false;       // 浮盈達門檻後將 SL 移至保本
input double          InpBreakEvenTriggerR = 1.0;         // 觸發門檻 = 初始風險 R 的倍數
input double          InpBreakEvenLockR    = 0.0;         // 鎖利距離 = R 的倍數 (0=純保本)，須 < Trigger
...
datetime lastBar   = 0;
datetime lastManageBar = 0;   // 部位管理獨立計數：不受訊號端資料就緒與否影響
...
//+------------------------------------------------------------------+
//| 保本停損：浮盈達 TriggerR 後把 SL 移到 entry ± LockR              |
//|                                                                  |
//|  觸發判定使用「已收盤棒」的 high/low (shift=1)，與訊號端同一鐵律： |
//|  結果不隨 tester tick model 改變，代價是最晚延遲一根 K 棒。        |
//|                                                                  |
//|  初始風險 R 由 |entry - SL| 推得，因此僅在 SL 仍位於虧損側時處理； |
//|  一旦移動完成 (SL 越過 entry) 即天然跳過，無需保存狀態。          |
//+------------------------------------------------------------------+
void ManageBreakEven()
  {
   if(!InpUseBreakEven) return;

   double barHigh[1], barLow[1];
   if(CopyHigh(_Symbol, _Period, 1, 1, barHigh) < 1) return;
   if(CopyLow(_Symbol, _Period, 1, 1, barLow) < 1)  return;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(sl <= 0.0 || entry <= 0.0) continue;      // 無 SL → 無法推得 R，不處理
      ...
      if(type == POSITION_TYPE_BUY)
        {
         if(sl >= entry) continue;                 // 已保本或已鎖利 → 不重複處理
         double r = entry - sl;
         if(barHigh[0] < entry + r * InpBreakEvenTriggerR) continue;
         newSL = AlignDown(entry + r * InpBreakEvenLockR);
         if(newSL <= sl) continue;                 // 不得使 SL 變差
         if((bid - newSL) < brokerMin) continue;   // 距現價過近 → 本根跳過，下一根再試
        }
      else if(type == POSITION_TYPE_SELL)
        {
         if(sl <= entry) continue;
         double r = sl - entry;
         if(barLow[0] > entry - r * InpBreakEvenTriggerR) continue;
         newSL = AlignUp(entry - r * InpBreakEvenLockR);
         if(newSL >= sl) continue;
         if((newSL - ask) < brokerMin) continue;
        }
      else
         continue;

      if(!trade.PositionModify(ticket, newSL, tp))
         PrintFormat("保本 PositionModify 失敗 ticket=%I64u newSL=%.5f: retcode=%u %s",
                     ticket, newSL, trade.ResultRetcode(), trade.ResultRetcodeDescription());
      else
         PrintFormat("保本移動 ticket=%I64u entry=%.5f SL %.5f→%.5f",
                     ticket, entry, sl, newSL);
     }
  }
...
int OnInit()
  {
   ...
   if(InpUseBreakEven && InpBreakEvenTriggerR <= 0.0)
     { Print("InpBreakEvenTriggerR 必須 > 0 (保本啟用時)");   return(INIT_PARAMETERS_INCORRECT); }
   if(InpUseBreakEven && InpBreakEvenLockR < 0.0)
     { Print("InpBreakEvenLockR 不可為負 (保本啟用時)");       return(INIT_PARAMETERS_INCORRECT); }
   // 鎖利距離 >= 觸發距離 → 新 SL 會落在觸發價之上(多)，等同觸發即停損，必屬設定錯誤
   if(InpUseBreakEven && InpBreakEvenLockR >= InpBreakEvenTriggerR)
     { Print("InpBreakEvenLockR 必須 < InpBreakEvenTriggerR"); return(INIT_PARAMETERS_INCORRECT); }
...
void OnTick()
  {
   ...
   // 部位管理先於訊號評估，且不因指標未就緒而被跳過 (已持倉的風險管理優先於新進場)
   if(curBar[0] != lastManageBar)
     {
      lastManageBar = curBar[0];
      ManageBreakEven();
     }
