...
狀態：BREAKOUT 模式已實作於 `Strategies/Strategy_Session_Range.mq5` 且編譯通過；baseline 已測，GBPUSD/EURUSD 否定，USDJPY M15 保留二輪驗證。結果見 [Strategy_Records/Strategy_Session_Range.md](../Strategy_Records/Strategy_Session_Range.md)。
...
- 沿用：`MaxSpread`。**Friday 提前收倉：已由每日 19:00 server（≈London 17:00）強平涵蓋**——本策略每天日內全平、無隔夜部位，19:00 遠早於 FX 週五收盤（≈23:00–24:00 server），故不需獨立的週五處理。若日後想要週五更早的 cutoff（例如避開 NFP 後展期），再加 `InpFridayCloseHour`。
...
6. ForceClose 時刻：平掉本 magic 全部持倉。**週五無需特判**——每日 19:00 server 強平已早於 FX 週五收盤，無隔夜/週末風險（見 §6）。
