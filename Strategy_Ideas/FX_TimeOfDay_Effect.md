...
狀態：Phase 0 broker cost gate 已正式 GO；conditional Phase 1 EA 已實作；EURUSD baseline formal backtest 未通過（2020–2026 子樣本轉負），策略暫停，不進 live/demo candidate（**有正式學術文獻支持，但零售 MT5 recent-regime edge 未成立**）
...
**目前進度（2026-07-07）**：已完成實作前閱讀。已新增並編譯通過 `Utilities/Script_FX_TimeOfDay_Cost_Check.mq5`（0 errors / 0 warnings）供 Phase 0 成本關卡使用；server 18:00 與 10:00 window 成本樣本皆為 `GO`，Phase 0 broker cost gate 正式通過。`Strategies/Strategy_Time_Window.mq5` 已完成 conditional Phase 1 實作並編譯通過（0 errors / 0 warnings），並在正式回測後補上 Friday window guard，避免週五尾盤缺 tick 時 Window B 持倉跨週末。EURUSD Phase 1 baseline 未通過：2015–2026 全樣本僅薄利，2020–2026 子樣本轉負，暫停本 FX idea，不進 live/demo candidate；後續 Gold Intraday Seasonality 仍可復用同一個已硬化的定時進出引擎，但必須獨立驗證。
...
**Phase 0 成本結果（2026-07-06 / 2026-07-07）**：

| Server time | Window proxy | Samples | Spread avg / median / max (pips) | Commission RT (pips) | Slippage RT (pips) | Round-turn cost (pips) | One-side equivalent (pips) | 判定 |
|---|---|---:|---:|---:|---:|---:|---:|---|
| 2026-07-06 17:45:53 | 18:00 entry window | 30 | 0.037 / 0.000 / 0.100 | 0.000 | 0.200 | 0.237 | 0.118 | GO |
| 2026-07-07 10:37:01 | 10:00 entry window | 30 | 0.043 / 0.000 / 0.300 | 0.000 | 0.200 | 0.243 | 0.122 | GO |

判讀：兩個實際進場窗口附近的 one-side equivalent cost 都遠低於 `0.8 pip` GO 門檻，Phase 0 broker cost gate 正式通過。Commission input 目前為 `0`；若補入常見 `7 USD / lot round-turn`（EURUSD pip value 約 `10 USD / pip / lot`），較差的 10:00 樣本成本約 `(0.043 + 0.700 + 0.200) / 2 = 0.472 pip`，仍屬 GO。若 round-turn commission 達 `14 USD / lot`，則約 `0.822 pip`，會落入 borderline。
...
| `InpAllowFullDayWindow` | bool / false | 預設禁止 open time = close time；Gold CTRL-3 可 opt-in 將其視為 24h window |
...
5. 週五：平倉時刻照常執行；週五收盤前所有窗口強制平倉（防隔週末）。若某個 Friday window 的預定 close time 等於或晚於 Friday force-close time，EA 會直接 skip 該 window，避免回測或實盤週五尾盤沒有 tick 時延到週一才平倉。
...
### Phase 1 baseline 結果（2026-07-07）

測試設定：`EURUSD / H1`，fixed lot `0.01`，real ticks，預設 Window A = server `10:00–18:00` SELL，Window B = server `18:00–23:00` BUY。

| 樣本 | Net Profit | Profit Factor | Sharpe | Max Equity DD | Trades | 判定 |
|---|---:|---:|---:|---:|---:|---|
| 2015.01.01–2026.06.30 | +309.39 | 1.044 | 0.457 | 279.75 / 2.64% | 5958 | 全樣本僅薄利，edge 不夠強 |
| 2020.01.01–2026.06.30 | -57.10 | 0.986 | -0.157 | 279.75 / 2.74% | 3368 | 未通過：recent-regime edge 不存在 |

Window 拆分：

| 樣本 | Window A net / PF | Window B net / PF | 判讀 |
|---|---:|---:|---|
| 2015–2026 | +260.32 / 1.057 | +49.65 / 1.019 | 利潤主要來自早期樣本與 Window A，但 PF 過低 |
| 2020–2026 | -2.22 / 0.999 | -54.61 / 0.962 | A 近乎歸零，B 明顯拖累 |

年度觀察：2015–2016 支撐大部分全樣本獲利；2017 已轉負，2018–2019 邊際，2020–2024 混合，2025 明顯失效（net `-230.50`、PF `0.676`），2026 YTD 仍為負。這符合 alpha decay / regime decay 的疑慮。

機制檢查：交易頻率大致正常（2020–2026 每日 2 筆，無多下單），但回測顯示 Window B 在部分 Friday `18:00` 進場後，因 Friday `23:00` 附近缺 tick，實際到 Monday `00:00` 或 `00:02` 才平倉。這違反「無週末持倉」驗收標準，因此 EA 已補 Friday window guard：Friday window 若預定 close time 等於或晚於 force-close time，直接 skip。

研究結論：本 FX Time-of-Day baseline 不通過第 9 節條件。`2020–2026` 子樣本 PF < 1 且 Sharpe < 0，沒有 economic significance；不建議做參數優化，避免在已失效樣本上 overfit。策略狀態改為 paused/rejected for FX，僅保留 `Strategy_Time_Window.mq5` 作為 Gold Intraday Seasonality 或其他純時間型研究的可復用引擎。
