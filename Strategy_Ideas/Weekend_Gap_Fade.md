...
| `InpSLMode` | GAP_MULT | SL = max(1.0×gap, 下限)；與 ATR(D1) 取較近者 |
| `InpForceCloseDay/Hour` | 週三 00:00 | 時間出場 |
| `InpFixedLots` / `InpMagic` | 0.01 / 770040 | |

**核心邏輯**：偵測「週開盤」（同 Phase 0 定義，用 bar 間隔 > 24h 判定）→ 計算 gap → 過濾（閾值/上限/點差平靜）→ 反 gap 方向市價單，TP = prev_close、SL 依模式 → 觸 TP/SL 或週三強平。每週最多 1 筆/symbol。閒置時間直接 return（本 EA 一週只活躍幾小時）。

**驗收標準**：編譯 0/0；tester 全年交易數 ≤ 52；每筆進場時間都在週開盤後 1 小時內；TP 價位 = 上週五收盤價（抽查 5 筆）；gap 超上限的週有「skipped: breakaway」log。
