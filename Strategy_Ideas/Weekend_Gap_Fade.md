...
狀態：Phase 0 short-sample 通過但樣本不足（MetaQuotes-Demo M15 只回到 2022-06）；依個人盤面觀察改走 **research prototype / demo-forward only** 路線，已新增 `Strategies/Strategy_Weekend_Gap.mq5` 供 demo 蒐集真實執行證據。最新 smoke / sensitivity 結論：GBPUSD、EURUSD 暫用 `InpMinGapATRMult=0.3` 小手數 demo forward，USDJPY 排除；尚未通過正式 Phase 1 backtest gate，**不得視為 live-ready**。
...
**目前進度（2026-07-06）**：已新增 prototype EA。此版本是 demo-forward evidence collector，不是正式通過統計關後的 live candidate。EA 已硬化至 live-ready 程式標準（輸入驗證 / 權限 / 保證金 / netting / 效能），仍待統計關。研究用途是記錄真實週一開盤執行品質，並驗證短樣本觀察是否延續。
...
### Phase 1.5：Smoke test / sensitivity 結論（2026-07-06）

測試環境：MetaQuotes-Demo，Build 5977；期間 2022.06.27–2026.06.30；M15，real ticks；固定 `0.01` lot；`InpTPMode=TP_FULL_FILL`，`InpMaxGapATRMult=1.5`。

Default `InpMinGapATRMult=0.3` smoke test：

| Symbol | Net profit | PF | Trades | Win rate | TP / SL / Force | 判讀 |
|---|---:|---:|---:|---:|---:|---|
| USDJPY | -15.28 | 0.00 | 3 | 0.0% | 0 / 3 / 0 | reject |
| GBPUSD | +30.46 | 3.17 | 11 | 72.7% | 8 / 3 / 0 | demo research candidate |
| EURUSD | +12.31 | 1.43 | 21 | 61.9% | 12 / 8 / 1 | weak demo research candidate |

`InpMinGapATRMult` sensitivity：

| Symbol | 0.0 | 0.1 | 0.2 | 0.3 | 結論 |
|---|---|---|---|---|---|
| GBPUSD | 42 trades, PF 1.18, +7.93 | 30 trades, PF 1.38, +13.94 | 16 trades, PF 2.20, +25.07 | 11 trades, PF 3.17, +30.46 | `0.3` 較乾淨；`0.2` 多出的邊際交易合計約 -5.39 |
| EURUSD | 65 trades, PF 0.95, -2.82 | 49 trades, PF 1.01, +0.42 | 26 trades, PF 1.13, +4.74 | 21 trades, PF 1.43, +12.31 | `0.3` 較佳；`0.2` edge 太薄 |

決策：

- 不再擴大 Weekend Gap optimizer；樣本太少，繼續切參數容易 data mining。
- Demo forward 僅限 evidence collection：GBPUSD / EURUSD 使用 `InpMinGapATRMult=0.3`、`InpFixedLots=0.01`；USDJPY 暫不掛。
- 對應 research preset：`../Strategy_Records/Strategy_Weekend_Gap_GBPUSD_DemoForward.set`、`../Strategy_Records/Strategy_Weekend_Gap_EURUSD_DemoForward.set`。
- 此策略仍不進 `Strategy_Live_Candidates/`，也不建立正式通過紀錄；需要更長 M15 歷史或足夠 forward 樣本才能重審。
